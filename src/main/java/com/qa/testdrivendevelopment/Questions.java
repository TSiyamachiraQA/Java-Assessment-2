...
public class Questions {
	...
	public int returnPosition(String input, char letter) {
		boolean present = input.indexOf(input) != -1 ? true : false;
		if (present) {
			System.out.println(input.indexOf(letter, 0));
	...
	public String[] removeDuplicates(String[] stringArray) {
		...
		ArrayList<String> aRandomList = new ArrayList<String>();

		/*
		 * Loop through array, add element to ArrayList only if was not added previously
		 */
		for (int i = 0; i < stringArray.length; i++) {

			if (!aRandomList.contains(stringArray[i])) {
				aRandomList.add(stringArray[i]);
			}

			// convert ArrayList back to array
			stringArray = aRandomList.toArray(new String[aRandomList.size()]);

			System.out.println(Arrays.toString(stringArray));

			System.out.println("Array after removing duplicates: " + Arrays.toString(stringArray));
		}
	...
	public int[] listGen() {
		...
		int randomnum4 = rand.nextInt(100);
		int randomnum5 = rand.nextInt(100);

		int[] ints = { randomnum1, randomnum2, randomnum3, randomnum4, randomnum5 };
...
//		return count; 
	...
	public boolean iBeforeE(String input) {
		...
		if (str.contains("cie")) {
			return false;
		} else if (str.contains("ie") || str.contains("cei")) {
			return true;
		} else {
			return false;
		}
